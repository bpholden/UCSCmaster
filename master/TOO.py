...
class TOO(threading.Thread):
    def __init__(self,sheetns="",test=False):
        ...
        self.exposure = self.apfucam['EXPOSURE']
        self.elapsed = self.apfucam['ELAPSED']

    def wait_end_exposure(self):

        timeleft = ( self.exposure.read(binary=True) - self.elapsed.read(binary=True) ) + 40
        rv = self.event.waitFor("== ExposureEnd",timeout=timeleft)
        if rv:
